...
contract WavGame is IWavGame, Ownable, ERC2771Recipient, ReentrancyGuard, ERC165 {
    ...
    function supportsInterface(bytes4 interfaceId) public view override returns (bool) {
        ...
        interfaceId == type(IWavContract).interfaceId ||
    ...
    function collect(address _recipient, address _owner, IDParam[] calldata _input) external override payable nonReentrant {
        uint totalPayable = 0;
        uint totalMinted = 0;
    ...
    function levelUp(address _owner, uint8 _nextLevel, IDParam[] memory _input) external override nonReentrant {
        ...
        for (uint256 i = 0; i < _input.length;) { // ID is array, incase levelUP from 1 => 2, can have diff ids
            if (EnumerableSet.contains(ownerToGames[_owner][_nextLevel - 1].burnableSet, _input[i].id)) {
                burnableIds[burnableIds.length] = _input[i].id;
                burnableAmountPerId[burnableAmountPerId.length] = _input[i].amount;
                totalBurnAmount += _input[i].amount;
            }
            unchecked {++i;} 
        }

        require(totalBurnAmount == nextLevel.base.requiredBurn, "REQUIRED_BURN_NOT_MET");
        wavContract.burnBatch(_msgSender(), burnableIds, burnableAmountPerId);
        // syncBurn(priorLevel, totalBurnAmount);
        priorLevel.base.burnCount += burnableAmountPerId.length;

        wavContract.mint(_msgSender(), nextLevel.mintableSet.values()[0], nextLevel.base.requiredMint, bytes(" "));
        syncMint(nextLevel, _msgSender(), nextLevel.base.requiredMint);

        emit LeveledUp(msg.sender, _msgSender(), _nextLevel, nextLevel.base.requiredMint);
    }
    ...
    //     IWavGame.Level[] memory _gameLevel = new Level(ownerToGames[_owner].length);
    //     for (uint256 i = 0; i < ownerToGames[_owner].length; i++) {
    //         _gameLevel[i] = ownerToGames[_owner][i];
    //     }
    //     return _gameLevel;
    // }

    // Get level by artist by id
    function getGameLevel(address _owner, uint8 _level) public view returns (IWavGame.BaseLevelInfo memory) {
        return ownerToGames[_owner][Helper.getLevelIndex(_level)].base;
    ...
    function syncMint(Level storage level, address _recipient, uint256 _mintCount) internal {
        level.base.mintCount += _mintCount;
    ...
    function wavMint(address _recipient, address _owner, uint8 _nextLevel, uint256 _id, uint256 _amount) external onlyOwner {
        require(ownerToGames[_owner][_nextLevel-1].mintableSet.contains(_id), "ID_NOT_MINTABLE_FOR_GIVEN_LEVEL");

        wavContract.mint(_recipient, _id, _amount, " ");
        syncMint(ownerToGames[_owner][_nextLevel-1], _recipient, _amount);
    ...
    function setRequiredBurn(address _owner, uint8 _level, uint8 _requiredBurn) external onlyOwner {
        ownerToGames[_owner][Helper.getLevelIndex(_level)].base.requiredBurn = _requiredBurn;
    }

    function setRequiredMint(address _owner, uint8 _level, uint8 _requiredMint) external onlyOwner {
        ownerToGames[_owner][Helper.getLevelIndex(_level)].base.requiredMint = _requiredMint;
    }

    function setPrizeCutOff(address _owner, uint8 _level, uint8 _prizeCutOff) external onlyOwner {
        ownerToGames[_owner][Helper.getLevelIndex(_level)].base.prizeCutOff = _prizeCutOff;
    }
    ...
    function setMintableSet(address _owner, uint8 _level, uint256[] calldata _mintableSet) external onlyOwner {
        for (uint256 i; i < _mintableSet.length;) {
            ownerToGames[_owner][Helper.getLevelIndex(_level)].mintableSet.add(_mintableSet[i]);
            unchecked {++i;} 
        }
    }	
